...
import { Send, User, Bot, AlertTriangle, FileText, CheckCircle2, Moon, Sun, X, Flag, LogOut, Home } from "lucide-react";
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const fetchInit = async () => {
      try {
        ...
        setMessages([{
          role: "bot",
          content: "Chào bạn! Tôi là HR Copilot — trợ lý nhân sự nội bộ. Bạn cần hỏi về vấn đề gì?",
        }]);
      }
    };
    fetchInit();
  ...
  const handleViewCitation = async (filename: string) => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/document/${filename}`);
  ...
  const handleReportError = async (msgIdx: number) => {
    ...
    try {
      await axios.post(`${API_BASE}/report-error`, {
      ...
      setReportNote("");
      alert("Đã gửi báo lỗi thành công! HR sẽ xem xét.");
    } catch {
  ...
  const handleSend = async (overrideMsg?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        is_escalated: res.data.is_escalated,
        citations: res.data.citations
      }]);
    } catch {
      setMessages(prev => [...prev, {
        role: "bot",
        content: "Lỗi kết nối tới Server. Đảm bảo Backend FastAPI đang chạy ở cổng 8000.",
        is_fallback: true
      }]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className={`min-h-screen font-sans flex items-center justify-center p-4 transition-colors duration-300 ${isDarkMode ? 'dark bg-gray-900' : 'bg-gray-100'}`}>
      <div className="w-full max-w-2xl bg-white dark:bg-slate-800 shadow-xl rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col transition-colors duration-300" style={{ height: "85vh" }}>
        ...
        <div className="bg-blue-600 dark:bg-slate-900 p-4 text-white flex justify-between items-center transition-colors duration-300">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-200">Xin chào, {user.fullname}</p>
            </div>
          </div>
          <div className="flex gap-2 items-center">
            ...
            <Link href="/" className="p-2 bg-blue-700 dark:bg-slate-800 hover:bg-blue-800 dark:hover:bg-slate-700 rounded-full transition" title="Trang chủ">
              <Home size={16} />
            </Link>
            <button onClick={handleLogout} className="p-2 bg-red-500/80 hover:bg-red-600 rounded-full transition" title="Đăng xuất">
              <LogOut size={16} />
            </button>
          </div>
        </div>
        ...
        {/* Chat Area */}
        <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
              ...
              <div className={`p-4 rounded-2xl shadow-sm text-sm w-full ${
                msg.role === 'user'
                  ...
                  : msg.is_fallback
                    ...
                    : msg.is_escalated
                      ? 'bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 text-gray-800 dark:text-amber-100 rounded-tl-none'
                      : 'bg-white dark:bg-slate-800 border dark:border-slate-700 text-gray-800 dark:text-slate-200 rounded-tl-none'
              }`}>
                {msg.is_fallback && <p className="text-red-700 dark:text-red-300 mb-2 font-semibold flex items-center gap-2"><AlertTriangle size={16} /> Ngoài phạm vi hỗ trợ</p>}
                {msg.is_escalated && <p className="text-amber-700 dark:text-amber-300 mb-2 font-semibold flex items-center gap-2"><Flag size={16} /> Đã chuyển yêu cầu tới HR</p>}
                ...
                {msg.role === 'bot' && !msg.is_fallback && index > 0 && (
                  <div className="mt-2 pt-2 border-t dark:border-slate-700 flex items-center gap-2">
                    {reportingIdx === index ? (
                      <div className="flex flex-col gap-2 w-full">
                        <input type="text" value={reportNote} onChange={(e) => setReportNote(e.target.value)} placeholder="Mô tả lỗi (VD: sai số ngày phép)..." className="text-xs px-3 py-2 border rounded-lg dark:bg-slate-900 dark:border-slate-600 dark:text-white focus:outline-none focus:ring-1 focus:ring-red-400" />
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => { setReportingIdx(null); setReportNote(""); }} className="text-xs px-3 py-1 bg-gray-200 dark:bg-slate-700 dark:text-slate-300 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-600 transition">Hủy</button>
                        </div>
                      </div>
                    ) : (
                      <button onClick={() => setReportingIdx(index)} className="text-xs text-gray-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 flex items-center gap-1 transition">
          ...
          {suggestions.length > 0 && messages.length <= 1 && (
            <div className="flex flex-wrap gap-2">
              {suggestions.map((q, i) => (
                <button key={i} onClick={() => handleSend(q)} className="text-xs px-3 py-2 bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-600 text-blue-600 dark:text-blue-400 rounded-full hover:bg-blue-50 dark:hover:bg-slate-700 transition">
                  {q}
                </button>
              ))}
            </div>
          ...
          {isLoading && (
            <div className="flex gap-3 max-w-[85%]">
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl rounded-tl-none border dark:border-slate-700 shadow-sm w-24 flex gap-1 justify-center items-center">
